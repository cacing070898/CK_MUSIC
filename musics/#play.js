...
module.exports.run = async (client, message, args, queue, searcher) => {
    ...
    if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
        try {
            ...
            {
                let strip = `----------------------------------------- `
                let erorPlaylist = new Discord.MessageEmbed()
                    .addField(strip, `Please insert a valid link or make sure that the playlist is visible\n${err}`)
                    .setColor("PURPLE")
                return message.channel.send(erorPlaylist);
            } 
    }
    else {
